...
For implementing register allocation, Appel's textbook proved to be extremely helpful, because contained pseudocode for a graph-coloring register allocation algorithm. That said, getting the implementation right was still tricky - the pseudocode itself spanned 7 pages (!) and we wrote a lot of tests to make sure the invariants were properly maintained. In the end we chose to make some modifications to the algorithm to make it more compatible with our existing code for spilling variables.

While implementing object-oriented features, figuring out how to add compiler-generated functions to initialize classes and global variables was pretty challenging. In the end we reverse-engineered some example assembly code generated by other compilers, which involved reading through assembly and figuring out how to translate it back into IR. 

Because I had no final exams during finals period, I left campus five days before the deadline for this project and worked remotely during that time. I can see now why some companies are against having people work remotely. Communication and coordination became much more difficult - even though we communicated via messaging and calling, some things are a lot easier to communicate when you're in the same room with a whiteboard.

Yet, despite all the challenges we faced, we worked hard and prevailed. While writing this reflection, I look back on last semester with a sense of relief and pride. Before I conclude, I'd like to leave a few tips and best practices for anyone who is thinking of taking compilers and wants the best chance of success. Think of this as a supplement to [How not to succeed in 4120]() Listed in no particular order:
...
- Speaking of late days, use them wisely. Late days have increasing penalties (-5/15/30/50 when I took this course), so if you have bugs it is often worth it to submit the project one day late and use the extra day to fix those bugs. Late days have a tendency to cascade, because they cause the next project to be started late. Since each project builds upon the last, if you submit a project incomplete you'll continue to be penalized for any broken/missing features in future projects.
